...
- (id)initWithArrayDatas:(NSArray*)data cellHeight:(CGFloat)cHeight heightTableView:(CGFloat)tHeightTableView x:(CGFloat)x y:(CGFloat)y width:(CGFloat)width refView:(UIView*)rView animation:(AnimationType)tAnimation openAnimationDuration:(CGFloat)openDuration closeAnimationDuration:(CGFloat)closeDuration{
	...
	//	CGRect refFrame = refView.frame;
		...
		self.view.frame = CGRectMake(x,y,width, heightTableView);
...
- (void)viewDidLoad {
	...
	uiTableView = [[UITableView alloc] initWithFrame:CGRectMake(-7,87,refFrame.size.width+paddingRight+13, 113) style:UITableViewStylePlain];
	
	uiTableView.dataSource = self;
	
	uiTableView.delegate = self;
	uiTableView.layer.masksToBounds=YES;
    uiTableView.layer.cornerRadius=3;
